...
// 新增：专门处理draft的函数
static int process_run_draft(const char *cmd, const char *proj, const char *log) {
    static const auto drafts = fs::absolute(fs::path(XMAKE) / "drafts");
    auto command = std::string("xmake ") + cmd + " -P \"" + drafts.string() + "\" " + proj;
    if (log) {
        command += " >> ";
        command += log;
        command += " 2>&1";
...
// 新增：专门测试draft的函数
static bool test_draft(int n, const char *log) {
    std::ofstream file;
    if (log) {
        file.open(log, std::ios::out | std::ios::app);
    }
    ...
    char str[] = "draftXX";
    std::sprintf(str, "draft%02d", n);

    os << "\x1b[36m" << str << " running" << "\x1b[0m" << std::endl
       << "==================" << std::endl;
    auto pass = process_run_draft("", str, log) == EXIT_SUCCESS && process_run_draft("run", str, log) == EXIT_SUCCESS;
    os << "=================" << std::endl
       << "\x1b[" << (pass ? 32 : 31) << 'm' << str << (pass ? " completed" : " failed") << "\x1b[0m" << std::endl
       << std::endl;
    return pass;
}
...
#error "Unsupported platform"
#endif
        pass = test_draft(n, null);
    } else {
        const auto path = fs::absolute(fs::path(XMAKE) / "log" / std::get<fs::path>(this->dst));
        const auto path_string = path.string();
        pass = test_draft(n, path_string.c_str());
    }
    {
        std::lock_guard lock(this->mutex);
        this->result.push_back(pass);
